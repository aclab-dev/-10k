...
class TestRegimeMultipliers:
    ...
    def test_unclear_is_most_conservative(self) -> None:
        unclear_mult = _REGIME_MULTIPLIERS[PrimaryRegime.UNCLEAR]
        for regime, mult in _REGIME_MULTIPLIERS.items():
            if regime != PrimaryRegime.UNCLEAR:
...
class TestRegimeAdjustment:
    ...
    def test_high_volatility_regime_reduces_cap(self) -> None:
        result = compute_dynamic_leverage(
            _vol_assessment(leverage_cap=10),
    ...
    def test_low_volatility_keeps_vol_cap(self) -> None:
        result = compute_dynamic_leverage(
    ...
    def test_unclear_regime_always_most_conservative(self) -> None:
        ...
        unclear_result = compute_dynamic_leverage(
            vol, PrimaryRegime.UNCLEAR, Environment.PAPER, _DEFAULT_LEVERAGE_CONFIG
        )
        for regime in PrimaryRegime:
...
class TestEnvironmentRegimeInteraction:
    ...
    def test_live_caps_low_volatility_regime(self) -> None:
        result = compute_dynamic_leverage(
            _vol_assessment(leverage_cap=10),
            PrimaryRegime.LOW_VOLATILITY,
            Environment.LIVE,
...
class TestEdgeCases:
    ...
    def test_higher_vol_cap_never_gives_lower_leverage_same_regime_env(self) -> None:
        """Monotonía: mayor vol_cap → mayor o igual leverage sugerido."""
        regime = PrimaryRegime.TRENDING
...
class TestDeterminismAndOutput:
    def test_same_input_same_output(self) -> None:
        ...
        r1 = compute_dynamic_leverage(
            vol, PrimaryRegime.RANGING, Environment.PAPER, _DEFAULT_LEVERAGE_CONFIG
        )
    ...
    def test_output_stores_regime_and_environment(self) -> None:
        ...
        assert result.regime == PrimaryRegime.BREAKOUT
    ...
    def test_vol_cap_in_output_matches_assessment(self) -> None:
        vol = _vol_assessment(leverage_cap=7)
        result = compute_dynamic_leverage(
            vol, PrimaryRegime.TRENDING, Environment.PAPER, _DEFAULT_LEVERAGE_CONFIG
        )
