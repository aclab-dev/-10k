...
signature = hmac.new(secret_key.encode(), params.encode(), "sha256").hexdigest()
...
> Los endpoints privados (account/trade) pueden tener límites más restrictivos por cuenta.
...
| `/openApi/swap/v2/trade/leverage`             | POST   | Cambiar leverage (`symbol, side`)    |
...
| `positionSide` | string  | `LONG` o `SHORT` (hedge mode)                         |
...
### 5.4 Listen Key (para WebSocket privado)
...
BingX ofrece cuentas demo directamente en su plataforma web (https://bingx.com), pero **no hay una API endpoint separada** para estas cuentas demo — utilizan las mismas credenciales del entorno de producción pero con fondos virtuales proporcionados por la plataforma.
...
4. **Hedge mode** — BingX soporta posiciones LONG y SHORT simultáneas (`positionSide`).
5. **Rate limit** — 500 req / 10s. En PAPER mode no aplica. En LIVE hay que implementar throttling.
6. **WebSocket keep-alive** — el listen key debe extenderse antes de expirar (PUT periódico).
